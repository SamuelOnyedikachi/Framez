...
export const usePostsStore = create<PostsState>((set, get) => ({
  ...
  fetchPosts: async () => {
    try {
      ...
      const transformedPosts = (data || []).map((post: PostWithCounts) => {
        const { likes, saved_posts, ...postData } = post;
        return {
          ...postData,
          likes_count: likes?.length || 0,
          comments_count: post.comments?.length || 0,
          is_liked: likes?.some((like) => like.user_id === user.id) || false,
          is_saved:
            saved_posts?.some((saved) => saved.user_id === user.id) || false,
  ...
  fetchSavedPosts: async () => {
    try {
      ...
      const savedPosts = (data || []).map((item: any) => {
        const post: PostWithCounts = item.post;
        const { likes, saved_posts, ...postData } = post;
        return {
          ...postData,
          likes_count: likes?.length || 0,
